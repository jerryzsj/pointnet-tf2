  ...
  with tf.compat.v1.variable_scope(scope) as sc:
      ...
      def get_deconv_dim(dim_size, stride_size, kernel_size, padding):
          ...
          if padding == 'VALID' and dim_size is not None:
            dim_size = dim_size + max(kernel_size - stride_size, 0)
          return dim_size
...
def max_pool2d_argmax(inputs,
  ...
  with tf.compat.v1.variable_scope(scope) as sc:
    kernel_h, kernel_w = kernel_size
    stride_h, stride_w = stride
    outputs, argmax = tf.nn.max_pool_with_argmax(inputs,
                             ksize=[1, kernel_h, kernel_w, 1],
                             strides=[1, stride_h, stride_w, 1],
                             padding=padding,
                             name=sc.name)
    print('max-pool indices', argmax)
...
def dropout(inputs,
  ...
  with tf.compat.v1.variable_scope(scope) as sc:
    outputs = tf.cond(is_training,
                      lambda: tf.nn.dropout(inputs, rate=1-keep_prob, noise_shape=noise_shape),
                      lambda: inputs)
    return outputs
